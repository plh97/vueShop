		...
		<ul class="lists">
          ...
          <ul class="goods-list">
            <li class="good-item" v-for="(goodItem,i) in orderItem.list" :key='i'>
              <img class="goodImg" :src="goodItem.goods_image" alt="">
              <div class="good-info">
...
export default {
  ...
  computed: {
    orderlist: () => {
      console.log("computed: orderlist");
      return store.state.orderlist.filter(arr => arr.status === "已退货");
    }
  },
  mounted() {
    console.log("mounted: lufaorderlist");
