		...
		<nav>
			...
			<div class="toggle">
				<div class="sortList">
					<p>品类:</p>
					<ul>
						<li v-for="(ti,i) in title" :data-id="ti.id" @click="getList " :key='i' :class="{'active': ti.id == act}">{{ti.name}}</li>
					</ul>
				</div>
			</div>
		</nav>
...
const {
	...
	export default {
		...
		created() {
			console.log('created');
			store.commit('list');
			// if(location.pathname.split("/").indexOf('fst') > 0){
			// 	this.source_list = store.state.fstList
			// }else if(location.pathname.split("/").indexOf('lufa') > 0){
			// 	this.source_list = store.state.lufa
			// }
		},
		methods: {
			getti(){
